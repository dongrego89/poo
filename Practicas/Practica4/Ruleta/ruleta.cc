...
void Ruleta::getEstadoRuleta(int &numeroJugadores,int &sumaDinero, int &lanzamientosBola, int &gananciasBanca){
	lanzamientosBola=this->lanzamientos_;
	gananciasBanca=this->banca_ - 1000000;
	sumaDinero=dineroJugadores_ + banca_;
	numeroJugadores=this->numeroJugadores_;
}


Ruleta::Ruleta(Crupier c) :	numeroJugadores_(0),bola_(-1), crupier_(c), banca_(1000000),lanzamientos_(0),dineroJugadores_(0) {
...
void Ruleta::leeJugadores(){
	...
	if(fichero.is_open()){
		...
		this->dineroJugadores_=0;
		this->numeroJugadores_=0;
		...
		while(fichero.getline(dni,300,',')){
			...
			this->dineroJugadores_+=atoi(dinero);
			this->numeroJugadores_+=1;
...
void Ruleta::getPremios(){
	...
	this->dineroJugadores_=0;//Se setea el sumatorio de dinero de jugadores a cero

	for(j=jugadores_.begin();j!=jugadores_.end();j++){//Ahora recorreremos jugador a jugador sus apuestas y actualizaremos el valor del dinero
		j->setApuestas();//Cargamos los valores de los ficheros
		...
		for(a=apuestas.begin();a!=apuestas.end();a++){
			...
			case 1://Apuesta sencilla
				...
				this->dineroJugadores_+=j->getDinero();
			break;
		...
		this->dineroJugadores_+=j->getDinero();//Actualizamos el valor del dinero del sumatorio de jugadores
	}
